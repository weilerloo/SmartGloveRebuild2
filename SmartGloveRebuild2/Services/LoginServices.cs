    ...
    public class LoginServices : ILoginService
    {
        public async Task<LoginResponse> Authenticate(LoginRequest loginRequest)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = await client.PostAsync("http://172.16.12.112:7006/api/Users/AuthenticateUser",
        ...
        public async Task<string> CheckRefreshToken(LoginRequest loginRequest)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = await client.GetAsync($"http://172.16.12.112:7006/api/Users/CheckUserActivity?UserName={loginRequest.UserName}&Password={loginRequest.Password}");
